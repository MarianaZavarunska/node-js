...
function readData() {
  fs.readdir(path.join(__dirname, "task-3"), (err, files1) => {
    if (err) {
      console.log(err);
      throw err;
    }
    files1.forEach((file1) => {
      console.log(file1);
      if (file1.isFile()) {
        fs.truncate(path.join(__dirname, "task-3", `${file1}`), 0, (err) => {
          if (err) {
            console.log(err);
            throw err;
          }
      ...
      fs.readdir(path.join(__dirname, "task-3", `${file1}`), (err, files2) => {
        if (err) {
          console.log(err);
          throw err;
        }
        files2.forEach((file2) => {
          ...
          if (file2.isFile()) {
            fs.truncate(
              path.join(__dirname, "task-3", `${file2}`, `${file2}`),
              0,
              (err) => {
                if (err) {
                  console.log(err);
            ...
            fs.rename(
              path.join(__dirname, "task-3", `${file2}`),
              path.join(__dirname, "task-3", `new${file2}`),
              (err) => {
                if (err) {
                  console.log(err);
                  throw err;
